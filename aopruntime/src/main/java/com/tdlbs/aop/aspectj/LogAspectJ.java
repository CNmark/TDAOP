...
import com.tdlbs.aop.annotation.Log;
...
public class LogAspectJ {
    ...
    @Pointcut("within(@com.tdlbs.aop.annotation.Log  *)")
    ...
    @Pointcut("execution(@com.tdlbs.aop.annotation.Log * *(..)) || methodInsideAnnotatedType()")
    public void method() {
    } //方法切入点

    @Pointcut("execution(@com.tdlbs.aop.annotation.Log *.new(..)) || constructorInsideAnnotatedType()")
    public void constructor() {
    } //构造器切入点


    @Around("(method() || constructor()) && @annotation(debugLog)")
    public Object logAndExecute(ProceedingJoinPoint joinPoint, Log debugLog) throws Throwable {
     ...
     * @param joinPoint
     */
    private void enterMethod(ProceedingJoinPoint joinPoint, Log debugLog) {
        if (!TDLogger.isDebug()) {
            return;
        }
        ...
        TDLogger.log(debugLog.priority(), Utils.getClassName(cls), builder.toString());
     ...
     */
    private void exitMethod(ProceedingJoinPoint joinPoint, Log debugLog, Object result, long lengthMillis) {
        if (!TDLogger.isDebug()) {
            return;
        }
